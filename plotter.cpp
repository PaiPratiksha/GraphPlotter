...
void drawArrowAxes() {
	...
	new_string.clear();
	new_string.push_back(n%10 + 48);
	new_string.push_back(46);
	n /= 10;
	
	//cout<<new_string[0]<<" "<<new_string[1]<<" ";
	
	while(n>0)
	{
	new_string.push_back(n%10 + 48);
	cout<<new_string.back()<<" ";
	n /= 10;
	}
	cout<<endl;
	...
	for (vector<char>::iterator it = new_string.begin(); it!= new_string.end(); it++){
		glRasterPos2f(xdisplay, -0.2f);
